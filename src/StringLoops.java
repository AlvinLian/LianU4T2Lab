public class StringLoops
{
    ...
    public int countCharacters(String character, String searchString) {
        String lowerWord = searchString.toLowerCase();
        String lowerChar = character.toLowerCase();
        int count = 0;
        for (int idx = 0; idx < lowerWord.length(); idx++ ) {
            if (lowerWord.substring(idx, idx + 1).equals(lowerChar)) {
                count++;
            }
        }
        return count;
    ...
    */
    public String reverseString(String origString) {
        ...
        for (int idx = origString.length() - 1; idx >= 0; idx --) {
            newString += origString.substring(idx, idx + 1);
        }
        return newString;
    }
}
